...
def test_valid_message_id():
    '''tests a valid message pin'''
    result = setup()
    new = message.message_send(result['token'], 1, 'Virgil')
    message_id = new['message_id']
    message.message_pin(result['token'], message_id)
    assert Data['channels'][0]['messages'][0]['is_pinned'] is True

def test_message_is_pinned():
    '''tests a message that is already pinned'''
    result = setup()
    new = message.message_send(result['token'], 1, 'Virgil')
    message_id = new['message_id']
    message.message_pin(result['token'], message_id)
    with pytest.raises(InputError):
        assert message.message_pin(result['token'], message_id)

def test_nonmember_access_error():
    '''tests unpinning a message by a non-member of a channel'''
    result = setup()
    new = message.message_send(result['token'], 1, 'Virgil')
    message_id = new['message_id']
    with pytest.raises(AccessError):
        assert message.message_pin(result['token2'], message_id)
