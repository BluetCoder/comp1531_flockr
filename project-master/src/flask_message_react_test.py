...
def test_message_react(url):
    ...
    payload = response.json()
    assert payload['messages'][0]['reacts'][0]['react_id'] == 1
    assert payload['messages'][0]['reacts'][0]['u_ids'] == [2,1]
    assert payload['messages'][0]['reacts'][0]['is_this_user_reacted'] == True
...
def test_reacted(url): 
    '''if user reacted before, rasie InputError'''
    # flask_set_up clears, registers a user and creates a channel
    result = helper.flask_set_up(url)
    token_1 = result['token']
    channel_id = result['channel_id']
